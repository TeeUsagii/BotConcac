module.exports.info = {
  ...
  description: {
    long: "Minigame Ma Sói",
    short: "Game Ma Sói"
  },
  group: "Admin",
  guide: [
		'[start/join/list/leave/test]',
	],
  countdown: 1,
  require: {
    "axios": "",
...
module.exports.handleEvents = async ({ event, api, Users, Others, Cherry }) => {
        const { senderID, threadID, body, messageID } = event;
        	...
        	if (!Cherry.ms) Cherry.ms = new Map();
        	if (!Cherry.ms.has(threadID)) return;
        	var values = Cherry.ms.get(threadID);
        	...
        	if (body.indexOf("Phanvai") == 0 || body.indexOf("Phân vai") == 0 || body.indexOf("phân vai") == 0) {
        	    ...
        	    for (var soi = 0; soi < values.sosoi.length; soi++) {
        	        ...
        	        const getms2 = vaitro.filter(vai => vai.Phe2 == "Ma Sói hoặc Dân Làng");
        	        const getms = vaitro.filter(vai => vai.Phe == "Ma Sói");
        	        const allms = getms2.concat(getms);
        	        const randomToUser = allms[Math.floor(Math.random() * allms.length)];
...
module.exports.run = async function ({ api, event, args, Cherry, Others }) {
	...
	if (args[0] == "create" || args[0] == "Create" || args[0] == "tạo" || args[0] == "Tạo") {
		...
		if (Cherry.ms.has(threadID)) return api.sendMessage("Hiện tại nhóm đã có phòng chơi Ma Sói", threadID, messageID);
		Cherry.ms.set(event.threadID, { 
		    ...
		    player: [ {
		        ...
		        "ready": false } ] });
		        console.log(values)
		return api.sendMessage("Phòng Ma Sói đã được tạo thành công.\nnhấn ms join để tham gia", threadID, messageID);
	}

	else if (args[0] == "join" || args[0] == "Join") {
		if (!values) return api.sendMessage("Hiện tại phòng Ma Sói chưa được tạo, bạn có thể tạo phòng bằng #ms create", threadID, messageID);
		...
		if (typeof values.player == "undefined") return api.sendMessage("Hiện tại chưa có phòng Ma Sói nào, bạn có thể tạo bằng cách sử dụng #ms create", threadID, messageID);
		...
		if (!values) return api.sendMessage("Hiện tại chưa có phòng Ma Sói nào, bạn có thể tạo bằng cách sử dụng #ms create", threadID, messageID);
		if (values.player.length <= 1) return api.sendMessage("Hiện tại phòng của bạn không có người chơi nào tham gia, bạn có thể mời người đấy tham gia bằng cách yêu cầu người chơi khác nhập #ms join", threadID, messageID);
	...
	else return Cherry.utils.throwError(this.config.name, threadID, messageID);
}
